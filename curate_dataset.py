...
def main():
    ...
    parser.add_argument('--paraphrase-demo', action='store_true', help='Run a small paraphrase demo on first 5 examples')
    ...
    if args.paraphrase_demo:
        ...
        for _, row in demo.iterrows():
            print("Original:", row['event_text'])
            print("Paraphrase:", paraphrase_text(row['event_text']), "\n")
        sys.exit(0)

    # --- Standardise
    df['output'] = df['output'].apply(lambda o: {
        **o,
    ...
    for _, row in df.iterrows():
        ...
        # augment_row returns a dict with 'event_text' and 'output'
    ...
    if args.paraphrase:
        print("Running paraphrase on augmented data...")
        paraphrase_rows = []
        for _, row in df_aug.iterrows():
            if random.random() < 0.3:
                para = paraphrase_text(row['event_text'])
                new_row = row.to_dict()
                new_row['event_text'] = para
                paraphrase_rows.append(new_row)
        print(f"Paraphrased {len(paraphrase_rows)} rows")
        if paraphrase_rows:
            df_aug = pd.concat([df_aug, pd.DataFrame(paraphrase_rows)], ignore_index=True).drop_duplicates(subset=['event_text'])
